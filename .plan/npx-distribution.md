# Plan: NPX Distribution + Universal IDE/CLI Coverage for opencode-mem
...
2. **IDE coverage is limited**: opencode-mem only supports Claude Code (plugin) and Cursor (hooks installer). The AI coding tools landscape has exploded — Gemini CLI (95k stars), OpenCode (110k stars), Windsurf (~1M users), Codex CLI, Antigravity, Goose, Crush, Copilot CLI, and more all support extensibility.
...
- **This npx CLI fully replaces the old `opencode-mem-installer`**: Not a supplement — the complete replacement.

## Solution

`npx opencode-mem` becomes a unified CLI: install, configure any IDE, manage the worker, search memory.

```
npx opencode-mem                          # Interactive install + IDE selection
npx opencode-mem install                  # Same as above
npx opencode-mem install --ide windsurf   # Direct IDE setup
npx opencode-mem start / stop / status    # Worker management
npx opencode-mem search <query>           # Search memory from terminal
npx opencode-mem transcript watch         # Start transcript watcher
```
...
2. **Create `src/npx-cli/index.ts`** — a Node.js CLI router (NOT Bun) with command categories:
   ...
   **Install commands** (pure Node.js, no Bun required):
   - `npx opencode-mem` or `npx opencode-mem install` → interactive install (IDE multi-select)
   - `npx opencode-mem install --ide <name>` → direct IDE setup (only for implemented IDEs; unimplemented ones error with "Support for <name> coming soon")
   - `npx opencode-mem update` → update to latest version
   - `npx opencode-mem uninstall` → remove plugin and IDE configs
   - `npx opencode-mem version` → print version

   **Runtime commands** (delegate to Bun via installed plugin):
   - `npx opencode-mem start` → spawns `bun worker-service.cjs start`
   - `npx opencode-mem stop` → spawns `bun worker-service.cjs stop`
   - `npx opencode-mem restart` → spawns `bun worker-service.cjs restart`
   - `npx opencode-mem status` → spawns `bun worker-service.cjs status`
   - `npx opencode-mem search <query>` → hits `GET http://localhost:37777/api/search?q=<query>`
   - `npx opencode-mem transcript watch` → starts transcript watcher

   **Runtime commands must check for installation first**: If plugin directory doesn't exist at `~/.claude/plugins/marketplaces/thedotmack/`, print "opencode-mem is not installed. Run: npx opencode-mem install" and exit.
...
3. **The install flow** (fully replaces git clone + build):
   ...
   - Copy `plugin/` to `~/.claude/plugins/cache/kykiles/opencode-mem/<version>/`
...
- `npx opencode-mem install` copies plugin to correct directories on macOS, Linux, and Windows
- Auto-detection finds installed IDEs
- `npx opencode-mem start/stop/status` work after install
- `npx opencode-mem search "test"` returns results
- `npx opencode-mem start` before install prints helpful error message
- `npx opencode-mem update` and `npx opencode-mem uninstall` work correctly
- `npx opencode-mem version` prints version
...
2. **Create Gemini CLI hooks installer** at `src/services/integrations/GeminiCliHooksInstaller.ts`:
   ...
   - Hook config format (verified against official docs):
     ```json
     {
       "hooks": {
         "AfterTool": [{
           "matcher": "*",
           "hooks": [{ "name": "opencode-mem", "type": "command", "command": "<path-to-hook-script>", "timeout": 5000 }]
         }]
       }
     }
     ```
   ...
   - Context injection via `~/.gemini/GEMINI.md` (append opencode-mem section with `<opencode-mem-context>` tags, same pattern as CLAUDE.md)
...
- `npx opencode-mem install --ide gemini-cli` merges hooks into `~/.gemini/settings.json`
...
- `GEMINI.md` gets opencode-mem context section
...
export const OpenCodeMemPlugin: Plugin = async (ctx) => {
...
return {
  tool: {
    opencode_mem_search: tool({
      description: "Search opencode-mem memory database",
...
1. **Create OpenCode plugin** at `src/integrations/opencode-plugin/index.ts`:
   ...
   | Mechanism | Event | Map to opencode-mem |
   |-----------|-------|-------------------|
   ...
   - Register `opencode_mem_search` custom tool using correct `tool({ description, args, execute })` API
...
3. **Create OpenCode setup in installer** (two options, prefer file-based):
   - **Option A (file-based):** Copy plugin to `~/.config/opencode/plugins/opencode-mem.ts` (auto-loaded at startup)
   - **Option B (npm-based):** Add to `~/.config/opencode/opencode.json` under `"plugin"` array: `["opencode-mem"]`
   - Config also supports JSONC (`opencode.jsonc`) and legacy `config.json`
   - Context injection: Append to `~/.config/opencode/AGENTS.md` (or create it) with `<opencode-mem-context>` tags
...
- `npx opencode-mem install --ide opencode` registers the plugin (file or npm)
...
- `opencode_mem_search` custom tool works in OpenCode sessions
...
| Event | Can Block? | Map to opencode-mem | Use |
|-------|-----------|-------------------|-----|
...
2. **Create Windsurf hooks installer** at `src/services/integrations/WindsurfHooksInstaller.ts`:
   ...
   - Context injection via `.windsurf/rules/opencode-mem-context.md` (workspace-level; Windsurf rules are workspace-scoped)
...
- `npx opencode-mem install --ide windsurf` creates hooks config at `~/.codeium/windsurf/hooks.json`
- Windsurf sessions are captured by the worker via post-action hooks
- `trajectory_id` is used as session identifier
- Context is injected via `.windsurf/rules/opencode-mem-context.md` (under 6K char limit)
...
2. **Create Codex setup in installer**:
   - Write transcript-watch config to `~/.opencode-mem/transcript-watch.json`
...
- `npx opencode-mem install --ide codex` creates transcript watch config
- Codex sessions appear in opencode-mem database
...
1. **Wire OpenClaw into the npx installer**:
   ...
   - Register in `~/.openclaw/openclaw.json` under `plugins.opencode-mem`
...
- `npx opencode-mem install --ide openclaw` registers the plugin
...
2. **Antigravity MCP setup**:
   ...
   - Context injection: `~/.gemini/GEMINI.md` (shared with Gemini CLI) and/or `.agent/rules/opencode-mem-context.md`
...
5. **Roo Code MCP setup**:
   ...
   - Context injection: `.roo/rules/opencode-mem-context.md`
...
- Each IDE can search opencode-mem via MCP tools
...
1. Remove `opencode-mem-installer` npm package (unpublish or mark deprecated with message pointing to `npx opencode-mem`)
2. Update `install/public/install.sh` → redirect to `npx opencode-mem`
...
4. `npx opencode-mem start/stop/status/search` all work
5. `npx opencode-mem update` updates correctly
6. `npx opencode-mem uninstall` cleans up all IDE configs
7. `npx opencode-mem version` prints version
8. `npx opencode-mem start` before install shows helpful error
