# opencode-mem Docker harness

A minimal container for exercising opencode-mem end-to-end without polluting your
...
| `build.sh` | Runs `npm run build` then `docker build`. Tag defaults to `opencode-mem:basic`. |
...
```bash
# From the repo root:
docker/opencode-mem/build.sh
docker/opencode-mem/run.sh
```

`run.sh` drops you into `bash` inside the container with `claude` on `PATH` and
the plugin pre-staged at `/opt/opencode-mem`. Launch it with:

```bash
claude --plugin-dir /opt/opencode-mem
```

On exit, the SQLite DB survives at `./.docker-opencode-mem-data/opencode-mem.db` on
the host — inspect with:

```bash
sqlite3 .docker-opencode-mem-data/opencode-mem.db 'select count(*) from observations'
```
...
running opencode-mem, not editing code.

On top of that:

- **Bun** (`/usr/local/bun`) — opencode-mem's worker service runtime
- **uv** (`/usr/local/bin/uv`) — provides Python for Chroma per `CLAUDE.md`
- **`plugin/`** copied to `/opt/opencode-mem` — the locally-built plugin tree
- **`/home/node/.claude`** and **`/home/node/.opencode-mem`** — pre-created mount points
...
```bash
docker run --rm -it \
  -v $(mktemp -d):/home/node/.opencode-mem \
  -e OPENCODE_MEM_CREDENTIALS_FILE=/auth/.credentials.json \
...
| Var | Where | Purpose |
|-----|-------|---------|
| `TAG` | `build.sh`, `run.sh` | Override image tag (default `opencode-mem:basic`). |
| `HOST_MEM_DIR` | `run.sh` | Override host path for the persistent `.opencode-mem` volume (default `$REPO_ROOT/.docker-opencode-mem-data`). |
| `ANTHROPIC_API_KEY` | `run.sh`, entrypoint | API-key auth. Skips the OAuth creds extraction. |
| `OPENCODE_MEM_CREDENTIALS_FILE` | entrypoint | Path (inside the container) to a mounted OAuth creds JSON. Copied to `$HOME/.claude/.credentials.json` at startup. |
...
```bash
docker/opencode-mem/run.sh claude --plugin-dir /opt/opencode-mem --print "what did we learn yesterday?"
```

## Cleanup

```bash
rm -rf .docker-opencode-mem-data   # wipes the persistent DB + Chroma store
docker rmi opencode-mem:basic       # removes the image
```
