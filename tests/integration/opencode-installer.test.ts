...
describe('OpenCode installer config registration', () => {
  ...
  it('adds opencode-mem to an existing plugin array', () => {
    const config = addOpenCodePluginReference({
      plugin: ['context-mode'],
      mcp: { context7: { enabled: true } },
    });

    expect(config.plugin).toEqual(['context-mode', './plugins/opencode-mem.js']);
    expect(config.mcp).toEqual({ context7: { enabled: true } });
  });

  it('does not duplicate an existing opencode-mem plugin reference', () => {
    const config = addOpenCodePluginReference({
      plugin: ['context-mode', './plugins/opencode-mem.js'],
    });
  ...
  it('removes only opencode-mem from plugin entries', () => {
  ...
  it('creates opencode.json when missing', () => {
    ...
    expect(config.plugin).toEqual(['./plugins/opencode-mem.js']);
  });
