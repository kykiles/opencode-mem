...
`)}debug(e,t,s,r){this.log(0,e,t,s,r)}info(e,t,s,r){this.log(1,e,t,s,r)}warn(e,t,s,r){this.log(2,e,t,s,r)}setErrorSink(e){P=e}error(e,t,s,r){this.log(3,e,t,s,r),this.routeErrorToSink(t,s,r)}routeErrorToSink(e,t,s){try{if(!P||!(s instanceof Error))return;P(s)}catch{}}dataIn(e,t,s,r){this.info(e,`\u2192 ${t}`,s,r)}dataOut(e,t,s,r){this.info(e,`\u2190 ${t}`,s,r)}success(e,t,s,r){this.info(e,`\u2713 ${t}`,s,r)}failure(e,t,s,r){this.error(e,`\u2717 ${t}`,s,r)}},d=new B;var K=require("crypto");function Q(a,e,t){return(0,K.createHash)("sha256").update([a||"",e||"",t||""].join("\0")).digest("hex").slice(0,16)}var E="opencode";function Se(a){return a.trim().toLowerCase().replace(/\s+/g,"-")}function L(a){if(!a)return E;let e=Se(a);return e?e==="transcript"?"codex":e.includes("opencode")?"opencode":e.includes("codex")?"codex":e.includes("cursor")?"cursor":e.includes("claude")?"claude":e:E}function J(a){let e=["opencode","claude","codex","cursor"];return[...a].sort((t,s)=>{let r=e.indexOf(t),n=e.indexOf(s);return r!==-1||n!==-1?r===-1?1:n===-1?-1:r-n:t.localeCompare(s)})}function z(a,e,t,s,r){let n=Date.now()-s,o=r!==void 0?"up.session_db_id = ?":"up.content_session_id = ?",i=r??e;return a.prepare(`
  ...
  `).get()!=null}function y(a,e,t){try{a.run(e)}catch(s){let r=s instanceof Error?s:new Error(String(s));throw d.warn("DB",`Failed to apply SQLite pragma ${t}`,{sql:e},r),s}}function re(a,e={}){let{enableWal:t=!0,enableIncrementalAutoVacuum:s=!0}=e;y(a,`PRAGMA busy_timeout = ${he}`,"busy_timeout"),y(a,"PRAGMA foreign_keys = ON","foreign_keys"),y(a,"PRAGMA synchronous = NORMAL","synchronous"),y(a,`PRAGMA journal_size_limit = ${ge}`,"journal_size_limit"),s&&!Oe(a)&&y(a,"PRAGMA auto_vacuum = INCREMENTAL","auto_vacuum"),t&&y(a,"PRAGMA journal_mode = WAL","journal_mode")}var j=class{db;constructor(e=q){e instanceof H.Database?this.db=e:(e!==":memory:"&&V(g),this.db=new H.Database(e)),re(this.db),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addObservationContentHashColumn(),this.addSessionCustomTitleColumn(),this.addSessionPlatformSourceColumn(),this.addObservationModelColumns(),this.ensureMergedIntoProjectColumns(),this.addObservationSubagentColumns(),this.addObservationsUniqueContentHashIndex(),this.addObservationsMetadataColumn(),this.dropDeadPendingMessagesColumns(),this.ensurePendingMessagesToolUseIdColumn(),this.dropWorkerPidColumn(),this.ensureSDKSessionsPlatformContentIdentity(),this.ensureUserPromptsSessionDbId(),this.ensurePendingMessagesSessionToolUniqueIndex()}getIndexColumns(e){return this.db.query(`PRAGMA index_info(${JSON.stringify(e)})`).all().map(t=>t.name)}hasUniqueIndexOnColumns(e,t){return this.db.query(`PRAGMA index_list(${e})`).all().some(r=>{if(r.unique!==1)return!1;let n=this.getIndexColumns(r.name);return n.length===t.length&&n.every((o,i)=>o===t[i])})}resolvePromptSessionDbId(e,t,s){if(t!==void 0)return t;let r=s?L(s):void 0;return r?this.db.prepare(`
    ...
    `).get(e,L(t))||null:this.db.prepare(`
        ...
        FROM observations
        WHERE id = ?
      `).get(e)||null}getObservationsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:r,project:n,platformSource:o,type:i,concepts:p,files:_}=t,u=s==="relevance",I=u?"":`ORDER BY o.created_at_epoch ${s==="date_asc"?"ASC":"DESC"}`,l=r&&!u?`LIMIT ${r}`:"",N=e.map(()=>"?").join(","),O=[...e],R=[];if(n&&(R.push("o.project = ?"),O.push(n)),o&&(R.push(`COALESCE(NULLIF(s.platform_source, ''), '${E}') = ?`),O.push(L(o))),i)if(Array.isArray(i)){let h=i.map(()=>"?").join(",");R.push(`o.type IN (${h})`),O.push(...i)}else R.push("o.type = ?"),O.push(i);if(p){let h=Array.isArray(p)?p:[p],b=h.map(()=>"EXISTS (SELECT 1 FROM json_each(o.concepts) WHERE value = ?)");O.push(...h),R.push(`(${b.join(" OR ")})`)}if(_){let h=Array.isArray(_)?_:[_],b=h.map(()=>"(EXISTS (SELECT 1 FROM json_each(o.files_read) WHERE value LIKE ?) OR EXISTS (SELECT 1 FROM json_each(o.files_modified) WHERE value LIKE ?))");h.forEach(A=>{O.push(`%${A}%`,`%${A}%`)}),R.push(`(${b.join(" OR ")})`)}let m=R.length>0?`WHERE o.id IN (${N}) AND ${R.join(" AND ")}`:`WHERE o.id IN (${N})`,f=this.db.prepare(`
      SELECT o.*
      FROM observations o
      LEFT JOIN sdk_sessions s ON s.memory_session_id = o.memory_session_id
      ${m}
    ...
    `).get(e,t)?.prompt_text??null}storeObservation(e,t,s,r,n=0,o,i){let p=this.storeObservations(e,t,[s],null,r,n,o,i);return{id:p.observationIds[0],createdAtEpoch:p.createdAtEpoch}}storeSummary(e,t,s,r,n=0,o){let i=o??Date.now(),p=new Date(i).toISOString(),u=this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,r||null,n,p,i);return{id:Number(u.lastInsertRowid),createdAtEpoch:i}}storeObservations(e,t,s,r,n,o=0,i,p){let _=i??Date.now(),u=new Date(_).toISOString();return this.db.transaction(()=>{let l=[],N=this.db.prepare(`
        ...
        RETURNING id
      `),O=this.db.prepare("SELECT id FROM observations WHERE memory_session_id = ? AND content_hash = ?");for(let m of s){let c=Q(e,m.title,m.narrative),f=N.get(e,t,m.type,m.title,m.subtitle,JSON.stringify(m.facts),m.narrative,JSON.stringify(m.concepts),JSON.stringify(m.files_read),JSON.stringify(m.files_modified),n||null,o,m.agent_type??null,m.agent_id??null,c,u,_,p||null,m.metadata??null);if(f){l.push(f.id);continue}let v=O.get(e,c);if(!v)throw new Error(`storeObservations: ON CONFLICT without existing row for content_hash=${c}`);l.push(v.id)}let R=null;if(r){let c=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,r.request,r.investigated,r.learned,r.completed,r.next_steps,r.notes,n||null,o,u,_);R=Number(c.lastInsertRowid)}return{observationIds:l,summaryId:R,createdAtEpoch:_}})()}getSessionSummariesByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:r,project:n,platformSource:o}=t,i=s==="relevance",p=i?"":`ORDER BY ss.created_at_epoch ${s==="date_asc"?"ASC":"DESC"}`,_=r&&!i?`LIMIT ${r}`:"",u=e.map(()=>"?").join(","),I=[...e],l=[];n&&(l.push("ss.project = ?"),I.push(n)),o&&(l.push(`COALESCE(NULLIF(s.platform_source, ''), '${E}') = ?`),I.push(L(o)));let N=l.length>0?`AND ${l.join(" AND ")}`:"",R=this.db.prepare(`
      SELECT ss.*
      ...
      ${p}
      ${_}
    `).all(...I);if(!i)return R;let m=new Map(R.map(f=>[f.id,f])),c=e.map(f=>m.get(f)).filter(f=>!!f);return r?c.slice(0,r):c}getUserPromptsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:r,project:n,platformSource:o}=t,i=s==="relevance",p=i?"":`ORDER BY up.created_at_epoch ${s==="date_asc"?"ASC":"DESC"}`,_=r?`LIMIT ${r}`:"",u=e.map(()=>"?").join(","),I=[...e],l=[];n&&(l.push("s.project = ?"),I.push(n)),o&&(l.push(`COALESCE(NULLIF(s.platform_source, ''), '${E}') = ?`),I.push(L(o)));let N=l.length>0?`AND ${l.join(" AND ")}`:"",R=this.db.prepare(`
      SELECT
        up.*,
        s.project,
        s.memory_session_id,
      ...
      ${p}
      ${_}
    `).all(...I);if(!i)return R;let m=new Map(R.map(c=>[c.id,c]));return e.map(c=>m.get(c)).filter(c=>!!c)}getTimelineAroundTimestamp(e,t=10,s=10,r,n){return this.getTimelineAroundObservation(null,e,t,s,r,n)}getTimelineAroundObservation(e,t,s=10,r=10,n,o){let i=o?L(o):void 0,p=(T,h)=>{let b=[],A=[];return n&&(b.push(`${T}.project = ?`),A.push(n)),i&&(b.push(`COALESCE(NULLIF(${h}.platform_source, ''), '${E}') = ?`),A.push(i)),{clause:b.length>0?`AND ${b.join(" AND ")}`:"",params:A}},_=p("o","src"),u=p("ss","src"),I=p("s","s"),l,N;if(e!==null){let T=`
      ...
      `,h=`
        ...
        LIMIT ?
      `;try{let b=this.db.prepare(T).all(t,..._.params,s),A=this.db.prepare(h).all(t,..._.params,r+1);if(b.length===0&&A.length===0)return{observations:[],sessions:[],prompts:[]};l=b.length>0?b[b.length-1].created_at_epoch:t,N=A.length>0?A[A.length-1].created_at_epoch:t}catch(b){return b instanceof Error?d.error("DB","Error getting boundary timestamps",{project:n},b):d.error("DB","Error getting boundary timestamps with non-Error",{},new Error(String(b))),{observations:[],sessions:[],prompts:[]}}}let O=`
      SELECT o.*
      FROM observations o
      LEFT JOIN sdk_sessions src ON src.memory_session_id = o.memory_session_id
