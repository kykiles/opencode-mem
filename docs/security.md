...
Server beta defaults to API-key auth. `OPENCODE_MEM_AUTH_MODE=local-dev` only enables the loopback development bypass when `OPENCODE_MEM_ALLOW_LOCAL_DEV_BYPASS=1` is also set; do not use it behind a reverse proxy or on a publicly reachable bind address.
