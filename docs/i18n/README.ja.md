...
または、Antigravity CLI用にインストールします([セットアップガイド](https://docs.opencode-mem.ai/antigravity-cli/setup)):
...
> **注:** Claude-MemはnpmにもPublishされていますが、`npm install -g opencode-mem`は**SDK/ライブラリのみ**をインストールします — プラグインフックの登録やワーカーサービスのセットアップは行われません。必ず上記の`npx opencode-mem install`または`/plugin`コマンドを使用してインストールしてください。

### 🦞 OpenClawゲートウェイ

[OpenClaw](https://openclaw.ai)ゲートウェイに、単一のコマンドで永続メモリプラグインとしてopencode-memをインストールできます:

```bash
curl -fsSL https://install.cmem.ai/openclaw.sh | bash
```

インストーラーは、依存関係、プラグインセットアップ、AIプロバイダー設定、ワーカー起動、そしてTelegram、Discord、Slackなどへのオプションのリアルタイム観察フィードを処理します。詳細は[OpenClaw統合ガイド](https://docs.opencode-mem.ai/openclaw-integration)を参照してください。
...
- **[使用ガイド](https://docs.opencode-mem.ai/usage/getting-started)** - Claude-Memが自動的に動作する仕組み
...
- **[プログレッシブディスクロージャー](https://docs.opencode-mem.ai/progressive-disclosure)** - Claude-Memのコンテキストプライミング戦略の背後にある哲学
...
- **[フックアーキテクチャ](https://docs.opencode-mem.ai/hooks-architecture)** - Claude-Memがライフサイクルフックを使用する方法
...
- **[リリースブランチ](https://docs.opencode-mem.ai/branches)** - stable、core-dev、community-edgeブランチのフロー
...
`community-edge`は、早期の信頼性修正やコミュニティ統合のためのソース実行ブランチです。ブランチフローと非安定版の実行手順については、**[リリースブランチ](https://docs.opencode-mem.ai/branches)**
を参照してください。
...
Claude-Memは、`OPENCODE_MEM_MODE`設定を通じて複数のワークフローモードと言語をサポートします。
...
`~/.opencode-mem/settings.json`にある設定ファイルを編集します:
...
ブランチ戦略とローカル実行手順については[リリースブランチ](https://docs.opencode-mem.ai/branches)を参照してください。
...
- **リポジトリ**: [github.com/kykiles/opencode-mem](https://github.com/kykiles/opencode-mem)
