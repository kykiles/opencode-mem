...
description: "Add persistent memory to Antigravity CLI with opencode-mem"
---
...
4. Register opencode-mem's MCP server in `~/.gemini/antigravity/mcp_config.json` **and** `~/.gemini/config/mcp_config.json`
5. Write a rules/context placeholder to `~/.agents/rules/opencode-mem-context.md`
...
<Tabs>
  <Tab title="Gemini API (Free)">
    ...
    cat > ~/.opencode-mem/settings.json << 'EOF'
    {
      ...
      "OPENCODE_MEM_GEMINI_API_KEY": "YOUR_API_KEY"
    }
    EOF
    ```
  ...
  <Tab title="OpenRouter">
    ...
    cat > ~/.opencode-mem/settings.json << 'EOF'
    {
      ...
      "OPENCODE_MEM_OPENROUTER_API_KEY": "YOUR_KEY"
    }
    EOF
    ```
  </Tab>
...
npx opencode-mem status

# Check hooks are installed — look for opencode-mem entries
cat ~/.gemini/settings.json | grep opencode-mem

# Check MCP registration in either candidate config path
cat ~/.gemini/antigravity/mcp_config.json | grep opencode-mem
cat ~/.gemini/config/mcp_config.json | grep opencode-mem
```
...
On session start, you'll see opencode-mem context injected with your recent observations and project history.
...
1. Verify hooks exist in settings:
   ...
   ```
   You should see entries like `"SessionStart"`, `"AfterTool"`, etc. with opencode-mem commands.
...
# View logs
npx opencode-mem logs

# Restart worker
npx opencode-mem restart
```
...
3. Check your AI provider is configured in `~/.opencode-mem/settings.json`

### Raw escape codes in output

If you see characters like `[31m` or `[0m` in the session context, your opencode-mem version may need updating — the Antigravity CLI adapter strips ANSI color codes automatically:
...
```bash
npx opencode-mem uninstall
```

This removes opencode-mem's hooks from `~/.gemini/settings.json`, cleans up the context section in `~/.gemini/GEMINI.md`, removes opencode-mem's entry from both MCP config files, and removes the rules context section — while preserving everything else in those files.
