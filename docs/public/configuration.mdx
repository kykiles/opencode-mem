...
|-------------------------------|---------------------------------|---------------------------------------|
| `OPENCODE_MEM_MODEL`            | `claude-haiku-4-5-20251001`     | Claude model used to compress observations (when using the Claude provider) |
| `OPENCODE_MEM_PROVIDER`         | `claude`                        | AI provider: `claude`, `gemini`, or `openrouter` |
| `OPENCODE_MEM_CLAUDE_AUTH_METHOD` | `subscription`                | Claude provider auth mode: `subscription`, `api-key`, or `gateway` |
...
| `OPENCODE_MEM_WORKER_PORT`      | `37700 + (uid % 100)`           | Worker service port (per-user default; override for fixed port) |
| `OPENCODE_MEM_WORKER_HOST`      | `127.0.0.1`                     | Worker service host address           |
| `OPENCODE_MEM_DATA_DIR`         | `~/.opencode-mem`                 | Data root — every other path (database, chroma, logs, settings.json, worker.pid) derives from this |
...
Gateway credentials live in `~/.opencode-mem/.env`, not `settings.json`.
...
Use [LiteLLM Gateway](configuration/litellm-gateway) when you want `OPENCODE_MEM_PROVIDER=claude` to route through LiteLLM while preserving the Claude Agent SDK worker path.
...
Configure which Claude model compresses your observations (only applies when `OPENCODE_MEM_PROVIDER=claude`).
...
`npx opencode-mem install` prompts for the Claude model (when the Claude provider is selected) and persists the choice to `~/.opencode-mem/settings.json`.
...
├── .install-version        # Version marker written by `npx opencode-mem install`/`repair`
...
- **HTTP API**: 10 endpoints on the worker service port (per-user, default `37700 + (uid % 100)`; see `~/.opencode-mem/settings.json`)
...
Access the settings modal from the web viewer. The worker prints its URL on startup; the port comes from `OPENCODE_MEM_WORKER_PORT`.
...
| **Worker Port** | `37700 + (uid % 100)` | Port for background worker service (override with `OPENCODE_MEM_WORKER_PORT`) |
...
The Setup hook never installs anything — runtime install (Bun, uv, `bun install`) happens in `npx opencode-mem install` / `npx opencode-mem repair` outside the session lifecycle.
...
If you specify an invalid Claude model name, the worker logs a warning and uses the default. Valid Claude models for `OPENCODE_MEM_MODEL`:
...
3. Verify new port:
   ```bash
   curl -s http://127.0.0.1:$OPENCODE_MEM_WORKER_PORT/api/health | jq .port
   ```
