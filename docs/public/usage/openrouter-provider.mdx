...
The full conversation history is sent to the model on each request. opencode-mem does
...
If OpenRouter errors, opencode-mem logs the failure and re-throws so the message stays pending for later retry. There is no Claude SDK fallback — earlier docs claimed automatic Claude fallback, but the wiring was never actually engaged in production (#2087). To switch providers, change `OPENCODE_MEM_PROVIDER` in settings.
...
HTTP-Referer: https://github.com/kykiles/opencode-mem
