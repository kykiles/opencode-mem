...
Knowledge agents let you compile a slice of your opencode-mem observation history into a **queryable "brain"** that answers questions conversationally. Instead of getting raw search results back, you get synthesized, grounded answers drawn from your actual project history -- decisions, discoveries, bugfixes, and features.
...
```
build_corpus name="hooks-expertise" query="hooks architecture" project="opencode-mem" limit=200
```
...
```bash
WORKER_PORT=$(jq -r .OPENCODE_MEM_WORKER_PORT ~/.opencode-mem/settings.json)
...
Under the hood, `CorpusBuilder` searches your observations, hydrates full records, parses structured fields (facts, concepts, files), calculates stats, and writes everything to `~/.opencode-mem/corpora/hooks-expertise.corpus.json`.
...
- **Model from settings**: Reads `OPENCODE_MEM_MODEL` from user settings -- no hardcoded model IDs
