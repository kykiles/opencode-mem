...
The OpenRouter provider is a generic OpenAI-compatible client. Set `OPENCODE_MEM_OPENROUTER_BASE_URL` to point it at any endpoint that speaks the OpenAI `/chat/completions` API — DeepSeek, a local LM Studio server, or any custom gateway. The model id you set in `OPENCODE_MEM_OPENROUTER_MODEL` is sent verbatim.

<Info>
**Base URL resolution:** When `OPENCODE_MEM_OPENROUTER_BASE_URL` is empty (default), requests go to OpenRouter unchanged. When set, opencode-mem POSTs to `<base_url>/chat/completions` — you can supply either a base like `https://api.deepseek.com/v1` (the `/chat/completions` path is appended automatically) or a full `.../chat/completions` URL (used verbatim). Trailing slashes are normalized. The environment variable `OPENROUTER_BASE_URL` is honored as a fallback.
</Info>
...
```json
{
  ...
  "OPENCODE_MEM_OPENROUTER_API_KEY": "YOUR_DEEPSEEK_API_KEY"
}
```
...
```json
{
  ...
  "OPENCODE_MEM_OPENROUTER_MODEL": "your-local-model-id"
}
```
