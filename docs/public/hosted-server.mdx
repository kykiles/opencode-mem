...
description: "Remote authenticated MCP recall, usage metering + quotas, and data deletion — how opencode-mem's cloud server works today."
---
...
The hosted server is the cloud side of opencode-mem: a Postgres-backed HTTP service
(`/v1`) plus a separate BullMQ generation worker. Where the local plugin keeps
memory in `~/.opencode-mem/opencode-mem.db` on your machine, the hosted server keeps
...
```
 ...
 │  - auth (api-key mode)       ├───────▶│  opencode-mem server         │
...
Set `OPENCODE_MEM_AUTH_MODE=api-key` and send `Authorization: Bearer <key>` on every
...
- **`GET /v1/connect`** (requires `memories:read`) returns the same command with a
  ...
  `OPENCODE_MEM_PUBLIC_URL` (recommended when behind a proxy or load balancer) or,
...
|---------|--------|------------------------|
| `OPENCODE_MEM_RATE_LIMIT_PER_MIN` | Max requests per **API key** per minute. | `429` with `Retry-After` and `X-RateLimit-*` headers. |
| `OPENCODE_MEM_MONTHLY_REQUEST_CAP` | Max requests per **team** per calendar month (UTC). | `402 quota_exceeded`. |
| `OPENCODE_MEM_MONTHLY_TOKEN_CAP` | Max provider **tokens** per team per month. Gates **writes only** — reads stay open so a team over budget can still recall. | `402` at the cap. |
| `OPENCODE_MEM_USAGE_METERING=1` | Records one `request` usage event per authenticated call (fire-and-forget). | — |
...
the separate BullMQ worker (`opencode-mem server worker start`) — the HTTP path
