...
description: "How opencode-mem intercepts file reads to save tokens using observation history"
---
...
Projects matching patterns in `OPENCODE_MEM_EXCLUDED_PROJECTS` skip the gate entirely. Configure this in `~/.opencode-mem/settings.json`:

```json
{
  "OPENCODE_MEM_EXCLUDED_PROJECTS": "/tmp/*,/scratch/*"
}
```
...
2. Find the opencode-mem hooks section under `hooks.PreToolUse` and remove the entry with the `Read` matcher.
...
The File Read Gate is one piece of opencode-mem's layered context strategy:
