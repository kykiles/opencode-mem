...
|------|--------|--------------|-------------------|
| **Stable** | `main` | Everyone. This is the `npx opencode-mem` install. | **Yes** - the only published line |
...
- **Just want memory that works?** Use stable. `npx opencode-mem@latest` is always `main`.
...
- An npm publish is what makes `npx opencode-mem@<version>` resolve.
