...
```bash
PORT=${OPENCODE_MEM_WORKER_PORT:-$(jq -r .OPENCODE_MEM_WORKER_PORT ~/.opencode-mem/settings.json)}
```
...
1. Find the worker port. The default is `37700 + (uid % 100)`. The configured port is the value of `OPENCODE_MEM_WORKER_PORT` in `~/.opencode-mem/settings.json`; the running worker also reports it on `/api/health`:
   ```bash
   PORT=$(jq -r .OPENCODE_MEM_WORKER_PORT ~/.opencode-mem/settings.json)
   lsof -i :$PORT
...
jq -r .OPENCODE_MEM_WORKER_PORT ~/.opencode-mem/settings.json
...
2. **Create issue**: [GitHub Issues](https://github.com/kykiles/opencode-mem/issues)
