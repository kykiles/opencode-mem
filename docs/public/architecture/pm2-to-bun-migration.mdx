...
This document describes the PM2 to Bun migration that occurred in v7.1.0 (December 2025). If you're installing opencode-mem for the first time, this migration has already been completed and you can use the current Bun-based system documented in the main guides.
...
- **Process Name**: `opencode-mem-worker`
...
- **Logs**: `~/.pm2/logs/opencode-mem-worker-*.log`
- **PID File**: `~/.pm2/pids/opencode-mem-worker.pid`

**Lifecycle Commands**:
```bash
pm2 start <script>           # Start worker
pm2 stop opencode-mem-worker   # Stop worker
pm2 restart opencode-mem-worker # Restart worker
pm2 delete opencode-mem-worker  # Remove from PM2
pm2 logs opencode-mem-worker    # View logs
```
...
- **Port File**: `~/.opencode-mem/.worker.port` (new)
...
- Database file: `~/.opencode-mem/opencode-mem.db` (same location)
...
if (!existsSync(pm2MigratedMarker)) {
  try {
    spawnSync('pm2', ['delete', 'opencode-mem-worker'], { stdio: 'ignore' });
...
`ensureWorkerRunning()` checks if `~/.opencode-mem/.worker.pid` exists (it doesn't for first run after update)
</Step>
<Step title="Start Worker Decision">
Worker not running → Call `startWorker()`
</Step>
<Step title="Migration Check">
Check if `~/.opencode-mem/.pm2-migrated` exists
</Step>
<Step title="PM2 Cleanup">
Execute `pm2 delete opencode-mem-worker` (errors ignored), create marker file
</Step>
...
**Location**: `~/.opencode-mem/.pm2-migrated`
...
4. **PM2 cleanup**: `pm2 delete opencode-mem-worker` (old worker terminated)
5. **Marker creation**: `~/.opencode-mem/.pm2-migrated` with timestamp
...
| `pm2 stop opencode-mem-worker` | `npm run worker:stop` | Stop worker |
| `pm2 restart opencode-mem-worker` | `npm run worker:restart` | Restart worker |
| `pm2 delete opencode-mem-worker` | `npm run worker:stop` | Remove worker |
| `pm2 logs opencode-mem-worker` | `npm run worker:logs` | View logs |
| `pm2 describe opencode-mem-worker` | `npm run worker:status` | Detailed status |
...
**Logs**:
```
Old: ~/.pm2/logs/opencode-mem-worker-out.log
     ~/.pm2/logs/opencode-mem-worker-error.log

New: ~/.opencode-mem/logs/worker-YYYY-MM-DD.log
```

**PID Files**:
```
Old: ~/.pm2/pids/opencode-mem-worker.pid

New: ~/.opencode-mem/.worker.pid
```
...
New: ~/.opencode-mem/.worker.pid
     ~/.opencode-mem/.worker.port
     ~/.opencode-mem/.pm2-migrated (all platforms)
```

**Database** (unchanged):
```
Same: ~/.opencode-mem/opencode-mem.db
```
...
├────┼────────────────────┼─────────┼─────────┼──────────┤
│ 0  │ opencode-mem-worker  │ online  │ 0       │ 2d 5h    │
└────┴────────────────────┴─────────┴─────────┴──────────┘
```
...
# Or just remove opencode-mem logs
rm -f ~/.pm2/logs/opencode-mem-worker-*.log
rm -f ~/.pm2/pids/opencode-mem-worker.pid
```
...
```
~/.opencode-mem/
├── opencode-mem.db          # Database (unchanged)
...
~/.pm2/
├── logs/
│   ├── opencode-mem-worker-out.log
│   └── opencode-mem-worker-error.log
├── pids/
│   └── opencode-mem-worker.pid
└── pm2.log
```
...
```
~/.opencode-mem/
├── opencode-mem.db          # Database (same file)
├── chroma/                # Vector embeddings (unchanged)
├── logs/
...
**Symptoms**:
- `pm2 list` still shows `opencode-mem-worker`
...
```bash
# Manual cleanup
pm2 delete opencode-mem-worker
pm2 save  # Persist the deletion

# Force re-migration (optional)
rm ~/.opencode-mem/.pm2-migrated
...
**Manual Resolution**:
```bash
rm ~/.opencode-mem/.worker.pid
rm ~/.opencode-mem/.worker.port
...
| `Invalid port X` | Port validation failed | Update `OPENCODE_MEM_WORKER_PORT` in settings |
...
# 2. Start PM2 worker
pm2 start plugin/scripts/worker-cli.js --name opencode-mem-worker
...
# 5. Verify migration
pm2 list  # Should NOT show opencode-mem-worker
cat ~/.opencode-mem/.pm2-migrated  # Should exist
