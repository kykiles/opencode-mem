...
```typescript
const workerPort = process.env.OPENCODE_MEM_WORKER_PORT ?? "<worker-port>"
...
(Runtime setup is handled out-of-band by `npx opencode-mem install` / `npx opencode-mem repair`. The Setup phase runs a sub-100ms `version-check.js` that prompts the user to repair if the `.install-version` marker is stale.)
