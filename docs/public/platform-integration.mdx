...
// or whatever the user set via OPENCODE_MEM_WORKER_PORT / settings.json. Read it from
// process.env.OPENCODE_MEM_WORKER_PORT, then ~/.opencode-mem/settings.json
// (OPENCODE_MEM_WORKER_PORT key), then fall back to the deterministic default.
...
```bash
OPENCODE_MEM_MODEL=claude-haiku-4-5-20251001  # Default Claude model for observations/summaries
OPENCODE_MEM_CONTEXT_OBSERVATIONS=50          # Observations injected at SessionStart
OPENCODE_MEM_WORKER_PORT=                     # Optional override; default = 37700 + (uid % 100)
OPENCODE_MEM_DATA_DIR=                        # Optional override for the data directory
...
<CardGroup cols={2}>
  ...
  <Card title="GitHub" icon="github" href="https://github.com/kykiles/opencode-mem">
