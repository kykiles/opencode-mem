...
the legacy `opencode-mem worker` for deployable use cases.
...
2. Generation can scale horizontally (`docker compose up --scale opencode-mem-worker=N`).
3. Restarting the HTTP server does not lose enqueued generation work — jobs
   live in Valkey, persisted by AOF.

The legacy `opencode-mem worker` runtime is **not** spawned in Docker. The
...
|-----------------------------------|----------|--------------------------------------------------------------|
| `OPENCODE_MEM_RUNTIME`              | Docker   | Must be `server-beta` in Docker (warned otherwise).          |
| `OPENCODE_MEM_QUEUE_ENGINE`         | Docker   | Must be `bullmq`. In-process queues are rejected in Docker.  |
| `OPENCODE_MEM_SERVER_DATABASE_URL`  | Always   | Postgres connection string. Fails fast at startup.           |
| `OPENCODE_MEM_REDIS_URL`            | bullmq   | Required when queue engine is `bullmq`.                      |
| `OPENCODE_MEM_AUTH_MODE`            | Always   | Must NOT be `local-dev` in Docker.                           |
| `OPENCODE_MEM_ALLOW_LOCAL_DEV_BYPASS` | Docker | Must NOT be `1`/`true` in Docker.                            |
| `OPENCODE_MEM_GENERATION_DISABLED`  | Optional | Set to `true` on the HTTP service when running a separate worker. |
| `OPENCODE_MEM_SERVER_PROVIDER`      | Worker   | One of `claude`, `gemini`, `openrouter`. Worker only.        |
...
## Generation worker mode (`opencode-mem server worker start`)

The same image runs the generation worker via:

```sh
opencode-mem server worker start
```
...
* Forces generation enabled even if `OPENCODE_MEM_GENERATION_DISABLED=true`
  is inherited from the shared compose file. The worker IS the generation
  process.

In Compose this is the `opencode-mem-worker` service. Scale it horizontally:

```sh
docker compose up -d --scale opencode-mem-worker=4
```
...
```sh
opencode-mem server api-key create \
  --name "ci"                  \
...
```sh
opencode-mem server api-key revoke <id>
```
...
> **Do not enable `OPENCODE_MEM_AUTH_MODE=local-dev` in Docker.** The
...
* `opencode-mem-server` — HTTP runtime.
  `OPENCODE_MEM_GENERATION_DISABLED=true` so the BullMQ Worker is **not**
  attached here.
* `opencode-mem-worker` — generation worker. Scale horizontally.
...
* Restart of `opencode-mem-server` and `opencode-mem-worker` mid-stream does
  not lose data.
* Revoking an API key denies subsequent reads and writes (401/403).
* No `worker-service.cjs` process runs in any container.
* `OPENCODE_MEM_AUTH_MODE=local-dev` is rejected inside Docker.
