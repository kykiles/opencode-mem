...
- `src/cli/handlers/context.ts` — remove the `platform === 'gemini-cli' || platform === 'gemini'` display branch (line 137). **Verify first** this branch is about hook-invoked platform display, not `OPENCODE_MEM_PROVIDER`/LLM-provider selection — do not touch anything reading `OPENCODE_MEM_PROVIDER`.
...
- `README.md` — remove the `npx opencode-mem install --ide gemini-cli` line (139-142), the "Restart Claude Code or Gemini CLI" mention (line 158), and the setup-doc link (line 194).
...
- `grep -ril "OPENCODE_MEM_GEMINI\|GeminiProvider\|GeminiObservationProvider" src/` still returns all the original Part-B hits — confirms the LLM provider was untouched.
...
The dev machine already has both Antigravity IDE and standalone Antigravity CLI installed and in active use, including a **pre-existing, live, working opencode-mem Gemini CLI hook installation** sharing the same config tree — this turned B0 from a hypothesis exercise into direct filesystem/binary inspection. No network installs were performed; no live `agy` session/model call was triggered (avoids spending the user's API quota / mutating their plugin state unasked).
...
4. **MCP config path — TWO real files exist, genuinely ambiguous, needs a dual-write**:
   - `~/.gemini/antigravity/mcp_config.json` (old path, already has opencode-mem registered from the existing MCP-only installer, real content, in use).
...
5. **Rules/context file path — CONFIRMED, existing code was already right**: `~/.agents/rules/` (**plural** "agents") is real and populated (`~/.agents/rules/opencode-mem-context.md` already exists from the current `ANTIGRAVITY_CONFIG` MCP installer). The earlier research citing singular `.agent/rules/` was wrong for this install — no `.agent/` (singular) directory exists anywhere. Keep the existing codebase's path assumption in `McpIntegrations.ts:129` as-is.
6. **GEMINI.md — CONFIRMED live**: `~/.gemini/GEMINI.md` is real, in active use (has native Antigravity "Gemini Added Memories" auto-entries plus opencode-mem's own `<opencode-mem-context>` tag block already injected and rendering correctly). No change needed to the context-injection target.
...
- `README.md` — add `npx opencode-mem install --ide antigravity` example and setup-doc link, replacing the removed Gemini CLI lines.
...
- `npm run build-and-sync` succeeds; `opencode-mem install --ide antigravity` runs clean in a scratch directory (no real Antigravity CLI required for this dry-run — just confirm the installer writes valid JSON/MD without throwing).
...
4. **Hard gate (per user's chosen risk approach — cannot be skipped or waved through by CI alone):** hands-on manual verification against the real, already-installed Antigravity CLI on this machine (`agy` v1.0.16, live `~/.gemini/` tree) —
   - Run the new `opencode-mem install --ide antigravity` for real on this machine and confirm it produces a superset/no-op-safe merge against the pre-existing live hooks (must not duplicate or corrupt the existing `opencode-mem`-named hook entries already in `~/.gemini/settings.json`).
   - Start a real `agy` session (interactive or `agy -p "..."` print mode), perform a tool call, confirm opencode-mem's hook fires (check worker logs / DB for a newly captured observation timestamped after the test run).
   ...
   - Confirm MCP registration: check whether `agy` actually reads `~/.gemini/antigravity/mcp_config.json`, `~/.gemini/config/mcp_config.json`, or both, by testing opencode-mem's MCP tools are reachable from a live `agy` session — this resolves the one dual-write ambiguity left open from B0.
